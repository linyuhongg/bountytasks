...
def parse_arguments():
    ...
    parser.add_argument(
        "-H", "--host", type=str, default="0.0.0.0", help="The address to submit task."
    )
    parser.add_argument(
        "-FP",
        "--fastapi_port",
    ...
    parser.add_argument(
        "-WS", "--workspace", type=str, default="./", help="Work space."
    )
...
async def submit_task(task: Task):
    ...
    if os.path.isfile(db_path):
        ...
        return {
            "msg": msg
        }  # TODO to align with return message when submit task successfully
...
def get_task_by_id(task_id: str):
    ...
    if os.path.isfile(db_path):
        ...
        cursor = conn.cursor()
        cursor.execute(
            r"select status, result, q_model_path from task where id=?", (task_id,)
        )
        res = cursor.fetchone()
        cursor.close()
        conn.close()
    return {
        "status": res[0],
        "optimized_result": deserialize(res[1]) if res[1] else res[1],
        "result_path": res[2],
    }
...
def get_task_status_by_id(request: Request, task_id: str):
    ...
    if not res:
        ...
        baseline = get_baseline_during_tuning(
            task_id, get_task_log_workspace(config.workspace)
        )
        tuning_result = get_res_during_tuning(
            task_id, get_task_log_workspace(config.workspace)
        )
        status = res[0]
...
async def read_logs(task_id: str):
    ...
    Yields:
        str: log lines
    """
    log_path = "{}/task_{}.txt".format(
        get_task_log_workspace(config.workspace), task_id
    )
...
class LogEventHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        log_path = "{}/task_{}.txt".format(
            get_task_log_workspace(config.workspace), self.task_id
        )
...
def start_log_watcher(websocket, task_id, last_position):
    ...
    observer.schedule(
        LogEventHandler(websocket, task_id, last_position), log_path, recursive=False
    )
    observer.start()
    return observer
...
async def websocket_endpoint(websocket: WebSocket, task_id: str):
    ...
    if not check_log_exists(
        task_id=task_id, task_log_path=get_task_log_workspace(config.workspace)
    ):
...
async def download_file(task_id: str):
    ...
    zip_filepath = os.path.abspath(
        os.path.join(get_task_workspace(config.workspace), task_id, zip_filename)
    )
