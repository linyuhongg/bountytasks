...
def query_task_result(task_id, db_path, workspace):
    ...
    if not res:
        ...
        baseline = get_baseline_during_tuning(
            task_id, get_task_log_workspace(workspace)
        )
    ...
    result = {
        "status": status,
        "tuning_information": tuning_info,
...
def is_valid_task(task: dict) -> bool:
    ...
    if (
        isinstance(task["optimized"], str)
        and task["optimized"] not in ["True", "False"]
    ) or (
        not isinstance(task["optimized"], str)
    ...
    if not isinstance(task["approach"], str) or task["approach"] not in [
        "static",
        "static_ipex",
        "dynamic",
        "auto",
